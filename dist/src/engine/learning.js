...
class LearningEngine {
     ...
     */
    async generateLessons(documentText, report, recipe, maxLessons = 3) {
        const topGaps = report.gaps.slice(0, maxLessons);
        const results = await Promise.all(topGaps.map((gap) => {
    ...
    async generateSingleLesson(documentText, gap, criterion) {
...
צור שיעור מיקרו-למידה מותאם אישית. השב ב-JSON לפי הסכמה:
{
  ...
  "after_from_doc": "שכתוב משופר של אותו הקטע תוך יישום הכלי"
        ...
        try {
            const raw = await this.llm.chatJSON(system_prompt_store_1.systemPromptStore.get('learning'), userMessage);
            return {
            ...
            console.warn(`[LearningEngine] LLM failed for criterion "${criterion.id}", using static fallback.`, error);
            return {
                criterion_id: criterion.id,
                criterion_question: criterion.question,
